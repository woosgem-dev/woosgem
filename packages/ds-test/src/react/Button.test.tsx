...
describe('Button', () => {
  describe('core 일치 검증', () => {
    ...
    it('TC-R105: loading prop이 core 결과와 일치한다', () => {
      ...
      // Core는 data-state='loading' 사용
      expect(btn).toHaveAttribute('data-state', coreAttrs['data-state']);
      expect(btn).toHaveAttribute('data-state', 'loading');
      // loading 상태에서도 disabled 속성이 설정됨
      expect(btn).toBeDisabled();
    });
  ...
  describe('커스터마이즈 오버라이드', () => {
    ...
    it('TC-O200: 보호 속성 data-variant 오버라이드 차단', () => {
      ...
      // 사용자의 data-variant="custom"이 무시되고 variant="outline"이 적용됨
      expect(btn).toHaveAttribute('data-variant', 'outline');
    });
    ...
    it('TC-O203: 보호 속성 data-state 오버라이드 차단', () => {
      // @ts-expect-error - 보호 속성 오버라이드 시도
    ...
    it('TC-O204: 보호 속성 data-full-width 오버라이드 차단', () => {
      // @ts-expect-error - 보호 속성 오버라이드 시도
      render(<Button data-full-width="custom" fullWidth={false}>Button</Button>);
      const btn = screen.getByRole('button');

      // fullWidth=false이므로 data-full-width가 없어야 함
